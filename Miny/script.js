...
async function gatherLooping() {
    ...
    try {
        ...
        if (data.data && data.data.cooldown && data.data.character) {
            ...
            setTimeout(() => {
               if (data.data.character.inventory[0].quantity == 80 && currentPosX == 2 && currentPosY == 0) {
                move(1, 5)
...
async function crafting(craftCode, quantity) {
  ...
  try {
      ...
      setTimeout(() => {
       if (data.data.character.inventory.some(item => item.quantity === 10) && currentPosX === 1 && currentPosY === 5) {
          move(4, 1)
